...
type Props = {
  title: string;
  description: string;
  reason: "OIL_PRESSING" | "GENERAL";
  isAdmin: boolean;
};

export function DecommissionedSection({ title, description, reason, isAdmin }: Props) {
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
      ...
      {loading ? (
        ...
        <p className="rounded-xl bg-stone-50 p-4 text-sm text-stone-600">Aucune entrée.</p>
      ) : (
        <>
          {selectedEntry ? (
            <EntryDetailCard
