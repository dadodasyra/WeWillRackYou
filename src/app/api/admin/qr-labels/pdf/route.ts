...
import { generateQrLabelPdf } from "@/lib/qr-label-pdf";
...
export async function GET(request: NextRequest) {
  ...
  const buffer = await generateQrLabelPdf({ from, to, baseUrl });

  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `attachment; filename="qr-labels-${from}-${to}.pdf"`,
    },
  });
}
